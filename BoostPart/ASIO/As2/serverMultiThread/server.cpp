...
public:
    ...
    void do_read() {
        ...
        /*
         *
         * 多线程的 server端，在此处，有两个异步动作，
         * 需要考虑线程安全
         *
         *

这也意味着对于一个客户端，只会有一个异步操作在等待。假如在某些情况，一个
客户端有两个异步方法在等待，你就需要互斥量了。这是因为两个等待的操作可能
正好在同一个时间完成，然后我们就会在两个不同的线程中间同时调用他们的完成
处理函数。所以，这里需要线程安全，也就是需要使用互斥量。 在我们的异步服务
端中，我们确实同时有两个等待的操作；
         *
         * */
    ...
    void update_clients_changed()
    {
        ...
        for( arrayclient::iterator b = copy.begin(), e = copy.end(); b !=e; ++b)
